...
class PNCPIngestionMessage(BaseModel):
    """Model for ingest from PNCP."""

    mode: PNCPIngestionMode = PNCPIngestionMode.UPDATE
    data_ini: Optional[datetime] = None
    data_fim: Optional[datetime] = None

    @validator("data_ini", "data_fim")
    def validate_backfill_dates(cls, v, values):
        """Validate that data_ini and data_fim are present for backfill mode."""
